...
public:
	...
	double next()
	{
		...
		return psfmt64[this->dsfmt.idx++] - one;
	}
	...
	void seed(uint32_t init_key[], int key_length)
	{
		...
		uint32_t r = ini_func1(psfmt32[idxof(0)] ^ psfmt32[idxof(mid % size)]
...
private:
	static const double one = 1.0;  ///< for converting [1,2) to [0,1)
	dsfmt_t dsfmt;                  ///< dsfmt internal state vector

	Random() { this->seed(time(0)); }
